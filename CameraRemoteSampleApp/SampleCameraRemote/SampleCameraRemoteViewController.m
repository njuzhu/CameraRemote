...
@implementation SampleCameraRemoteViewController {
    ...
    NSInteger _singleExecuteTime;
    BOOL _isTaken;
    NSInteger _continuousExecuteTime;
}

@synthesize modeButtonText;
@synthesize continuousShootingText;
@synthesize singleShootingText;
@synthesize actionButtonText;
@synthesize higherActionButtonText;
@synthesize normalActionButtonText;
@synthesize lowerActionButtonText;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    _singleExecuteTime = 0;
    _continuousExecuteTime = 0;
    ...
    [continuousShootingText setHidden:YES];
    continuousShootingText.tag = -1;
    [singleShootingText setHidden:YES];
    singleShootingText.tag = -1;
    [actionButtonText setHidden:YES];
    actionButtonText.tag = -1;
    [higherActionButtonText setHidden:YES];
    [normalActionButtonText setHidden:YES];
    [lowerActionButtonText setHidden:YES];
    [self setButtonView:modeButtonText];
    [self setButtonView:continuousShootingText];
    [self setButtonView:singleShootingText];
    [self setButtonView:actionButtonText];
    [self setButtonView:higherActionButtonText];
    [self setButtonView:normalActionButtonText];
    [self setButtonView:lowerActionButtonText];
...
//    [SampleCameraApi setExposureCompensation:self exposureCompensation:@"-5"];
}
...
- (IBAction)modeButton:(id)sender
{
    ...
    [continuousShootingText setEnabled:NO];
    [singleShootingText setEnabled:NO];
    [actionButtonText setEnabled:NO];
    [higherActionButtonText setEnabled:NO];
    [normalActionButtonText setEnabled:NO];
    [lowerActionButtonText setEnabled:NO];
...
{
    NSLog(@"continuousShootingButton");
//    [SampleCameraApi getAvailableApiList:self isSync:NO];
//    [SampleCameraApi getSupportedExposureCompensation:self isSync:NO];
//    [SampleCameraApi getAvailableExposureCompensation:self isSync:NO];
//    [SampleCameraApi setExposureCompensation:self exposureCompensation:@"6" isSync:NO];
//    [SampleCameraApi actTakePicture:self];
//    [self progressIndicator:YES];
//    sleep(5);
//    [SampleCameraApi setExposureCompensation:self exposureCompensation:@"0" isSync:NO];
//    [SampleCameraApi actTakePicture:self];
//    [self progressIndicator:YES];
//    sleep(5);
//    [SampleCameraApi setExposureCompensation:self exposureCompensation:@"-6" isSync:NO];
//    [SampleCameraApi actTakePicture:self];
//    [self progressIndicator:YES];
    _continuousExecuteTime = 1;
    [self setHigherEC];
//    [NSTimer scheduledTimerWithTimeInterval:0 target:self selector:@selector(setHigherEC:) userInfo:nil repeats:NO];
//    [NSTimer scheduledTimerWithTimeInterval:10 target:self selector:@selector(setNormalEC:) userInfo:nil repeats:NO];
//    [NSTimer scheduledTimerWithTimeInterval:20 target:self selector:@selector(setLowerEC:) userInfo:nil repeats:NO];
//    [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(returnNormalEC:) userInfo:nil repeats:NO];
}

- (IBAction)singleShootingButton:(id)sender
{
    NSLog(@"singleShootingButton");
//    [SampleCameraApi getAvailableApiList:self isSync:YES];
//    [SampleCameraApi actTakePicture:self];
//    [self progressIndicator:YES];
    switch (_singleExecuteTime) {
        case 0:
            [self setHigherEC];
            _singleExecuteTime = 1;
            break;
        case 1:
            [self setNormalEC];
            _singleExecuteTime = 2;
            break;
        case 2:
            [self setLowerEC];
            _singleExecuteTime = 0;
            [NSTimer scheduledTimerWithTimeInterval:10 target:self selector:@selector(returnNormalEC:) userInfo:nil repeats:NO];
            break;
        default:
            _singleExecuteTime = 0;
            break;
...
- (void)setNormalEC:(NSTimer*)timer {
    [SampleCameraApi setExposureCompensation:self exposureCompensation:@"0" isSync:NO];
//    [SampleCameraApi actTakePicture:self];
//    [self progressIndicator:YES];
    _isTaken = YES;
...
- (void)returnNormalEC:(NSTimer*)timer {
    [SampleCameraApi setExposureCompensation:self exposureCompensation:@"0" isSync:NO];
}
...
//        [SampleCameraApi setExposureCompensation:self exposureCompensation:@"-5"];
//        [SampleCameraApi setContShootingMode:self contShootingMode:@"Continuous"];
        [SampleCameraApi actTakePicture:self];
        [self progressIndicator:YES];
    }
}
...
- (void)pickerTap
{
    ...
    [continuousShootingText setEnabled:YES];
    [singleShootingText setEnabled:YES];
    [actionButtonText setEnabled:YES];
    [higherActionButtonText setEnabled:YES];
    [lowerActionButtonText setEnabled:YES];
    [normalActionButtonText setEnabled:YES];
}
...
//    [SampleCameraApi getAvailableExposureCompensation:self];
//    [SampleCameraApi getAvailableShootMode:self];
//    [SampleCameraApi getContShootingMode:self];
//    [SampleCameraApi getContShootingSpeed:self];
//    [SampleCameraApi getExposureMode:self];
}
...
- (void)didCameraStatusChanged:(NSString *)status
{
    ...
    if ([PARAM_CAMERA_cameraStatus_movieRecording isEqualToString:status]) {
        ...
        [continuousShootingText setHidden:NO];
        [continuousShootingText setEnabled:YES];
        [continuousShootingText setBackgroundColor:[UIColor whiteColor]];
        [singleShootingText setHidden:NO];
        [singleShootingText setEnabled:YES];
        [singleShootingText setBackgroundColor:[UIColor whiteColor]];
        [higherActionButtonText setHidden:NO];
        [higherActionButtonText setEnabled:YES];
        [higherActionButtonText setBackgroundColor:[UIColor whiteColor]];
        [normalActionButtonText setHidden:NO];
        [normalActionButtonText setEnabled:YES];
        [normalActionButtonText setBackgroundColor:[UIColor whiteColor]];
        [lowerActionButtonText setHidden:NO];
        [lowerActionButtonText setEnabled:YES];
        [lowerActionButtonText setBackgroundColor:[UIColor whiteColor]];
    }
    ...
    if ([PARAM_CAMERA_cameraStatus_stillCapturing isEqualToString:status] &&
        [@"still" isEqualToString:_currentShootMode]) {
        [continuousShootingText setEnabled:NO];
        [continuousShootingText setBackgroundColor:[UIColor grayColor]];
        [singleShootingText setEnabled:NO];
        [singleShootingText setBackgroundColor:[UIColor grayColor]];
        [actionButtonText setEnabled:NO];
        [actionButtonText setBackgroundColor:[UIColor grayColor]];
        [higherActionButtonText setEnabled:NO];
        [higherActionButtonText setBackgroundColor:[UIColor grayColor]];
        [normalActionButtonText setEnabled:NO];
        [normalActionButtonText setBackgroundColor:[UIColor grayColor]];
        [lowerActionButtonText setEnabled:NO];
        [lowerActionButtonText setBackgroundColor:[UIColor grayColor]];
    }
...
- (void)setInitialShootModeUI
{
    ...
    [continuousShootingText setEnabled:YES];
    [continuousShootingText setBackgroundColor:[UIColor whiteColor]];
    [continuousShootingText setTitle:@"连续阶段曝光"forState:UIControlStateNormal];
    [singleShootingText setHidden:NO];
    [singleShootingText setEnabled:YES];
    [singleShootingText setBackgroundColor:[UIColor whiteColor]];
    [singleShootingText setTitle:@"单拍阶段曝光" forState:UIControlStateNormal];
    [higherActionButtonText setHidden:NO];
    [higherActionButtonText setEnabled:YES];
    [higherActionButtonText setBackgroundColor:[UIColor whiteColor]];
    [higherActionButtonText setTitle:@"Higher" forState:UIControlStateNormal];
    [normalActionButtonText setHidden:NO];
    [normalActionButtonText setEnabled:YES];
    [normalActionButtonText setBackgroundColor:[UIColor whiteColor]];
    [normalActionButtonText setTitle:@"Normal" forState:UIControlStateNormal];
    [lowerActionButtonText setHidden:NO];
    [lowerActionButtonText setEnabled:YES];
    [lowerActionButtonText setBackgroundColor:[UIColor whiteColor]];
    [lowerActionButtonText setTitle:@"Lower" forState:UIControlStateNormal];
    ...
    if ([@"movie" isEqualToString:_currentShootMode]) {
        ...
        [actionButtonText setBackgroundColor:[UIColor grayColor]];
        actionButtonText.tag = -1;
        [continuousShootingText setTitle:@"" forState:UIControlStateNormal];
        [continuousShootingText setEnabled:NO];
        [continuousShootingText setBackgroundColor:[UIColor grayColor]];
        [singleShootingText setTitle:@"" forState:UIControlStateNormal];
        [singleShootingText setEnabled:NO];
        [singleShootingText setBackgroundColor:[UIColor grayColor]];
        [higherActionButtonText setTitle:@"" forState:UIControlStateNormal];
        [higherActionButtonText setEnabled:NO];
        [higherActionButtonText setBackgroundColor:[UIColor grayColor]];
        [normalActionButtonText setTitle:@"" forState:UIControlStateNormal];
        [normalActionButtonText setEnabled:NO];
        [normalActionButtonText setBackgroundColor:[UIColor grayColor]];
        [lowerActionButtonText setTitle:@"" forState:UIControlStateNormal];
        [lowerActionButtonText setEnabled:NO];
        [lowerActionButtonText setBackgroundColor:[UIColor grayColor]];
    }
    [modeButtonText
...
- (void)parseMessage:(NSData *)response apiName:(NSString *)apiName
{
    ...
    if ([apiName isEqualToString:API_CAMERA_getAvailableApiList]) {
    ...
    } else if ([apiName isEqualToString:API_CAMERA_setExposureCompensation]
               && errorCode == -1 && _isTaken) {
        _isTaken = NO;
        [SampleCameraApi actTakePicture:self];
        [self progressIndicator:YES];
        ...
        [self parseActTakePicture:resultArray
                        errorCode:errorCode
                     errorMessage:errorMessage];
        if (_continuousExecuteTime != 0) {
            switch (_continuousExecuteTime) {
                case 1:
                    [self setNormalEC];
                    _continuousExecuteTime = 2;
                    break;
                case 2:
                    [self setLowerEC];
                    _continuousExecuteTime = 3;
                    break;
                case 3:
                    [self returnNormalEC];
                    _continuousExecuteTime = 0;
